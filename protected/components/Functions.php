...
class Functions extends CApplicationComponent {
    ...
    public function Services() {
        return array(
            ...
            4 => t("Delivery / Pickup / Online"),
            5 => t("Delivery & Online/Digital"),
            6 => t("Pickup & Online/Digital"),
            7 => t("Online/Digital Only")
        );
    }
    ...
    public function jsLanguageAdmin() {
        ...
        return array(
            ...
            'dinein_time_is_required' => t("Delivery Time is required"),
            'dinein_time' => t("Delivery Time")
        );
    }
    ...
    public function merchantMenu() {
        ...
        $payment_list = array('visible' => $this->hasMerchantAccess("payment-gateway"), 'tag' => 'payment-gateway', 'label' => '<i class="fa fa-usd"></i>' . Yii::t("default", 'Payment Gateway'),
            'itemOptions' => array('class' => ''), 'items' => array(
                ...
                array('visible' => $this->hasMerchantAccess("mps"), 'tag' => 'mpesa', 'label' => '<i class="fa fa-money"></i>' . Yii::t("default", "Lipa na MPESA"),
                    'url' => array('merchant/mpesaSettings')),
            ...
            /* array('visible'=>$this->hasMerchantAccess("ip8"),'tag'=>'ip8', 'label'=>'<i class="fa fa-paypal"></i>'.Yii::t("default","Ipay88"), 
              ...
              'url'=>array('merchant/moneris')), */
                ...
                array('visible'=>$this->hasMerchantAccess("ipay_africa"),'tag'=>'ipay_africa',
                    'label'=>'<i class="fa fa-paypal"></i>'.t('Ipay Africa'),
                    'url'=>array('merchant/ipay_africa')),
					array('visible'=>$this->hasMerchantAccess("mpesa_checkout"),'tag'=>'mpesa_checkout',
                    'label'=>'<i class="fa fa-paypal"></i>'.t('M-Pesa Checkout'),
                    'url'=>array('merchant/mpesa_checkout')),
        ...
        return array(
            ...
            'items' => array(
                ...
                array('visible' => $this->AA('merchant'),
                    'tag' => "merchant", 'label' => '<i class="fa fa-briefcase"></i>' . Yii::t("default", "Merchant List"),
                ...
                array('visible' => $this->AA('paymentgateway'), 'tag' => 'paymentgateway',
                    ...
                    'itemOptions' => array('class' => ''), 'items' => array(
                        ...
                        array('visible'=>$this->AA('ipay_africa'),'tag'=>'ipay_africa',
                            'label'=>'<i class="fa fa-paypal"></i>'.t("Ipay Africa"),
                            'url'=>array('admin/ipay_africa')),
                        
                        array('visible' => $this->AA('mpesaSettings'),
                            'tag' => 'mpesaSettings', 'label' => '<i class="fa fa-paypal"></i>' . Yii::t("default", "Lipa na MPESA"),
                            'url' => array('admin/mpesaSettings')),
    ...
    public function topMenu() {
        ...
        return array(
            ...
            'items' => array(
                ...
                array('visible' => $enabled_reg, 'tag' => "home", 'label' => '<i class="fa fa-briefcase"></i>' . Yii::t("default", "Restaurant Signup"),
    ...
    public function updateOptionAdmin($option_name = '', $option_value = '') {
        ...
        if (is_array($rows) && count($rows) >= 1) {
            ...
            $res = $command->update('{{option}}', $params, 'id=:id', array(':id' => addslashes($option_id)));
            ...
            if (is_array($cart_item) && count($cart_item) >= 1) {
                ...
                foreach ($cart_item as $key => $val) {
                    ...
                    if ($res_category = $this->getCategory($val['category_id'])) {
                        ...
                        $htm .= "<p style=\"margin:0;\"><b>" . qTranslate($res_category['category_name'], 'category_name', $category_name_trans) . "</b></p>";
                    }
                        ...
                        foreach ($val['sub_item'] as $cat_id => $val_sub) {
                            ...
                            if (array_key_exists($cat_id, (array) $subcat_list)) {
                                ...
                                $htm .= '<div class="b uk-text-success">' .
                                        qTranslate($subcat_list[$cat_id], 'subcategory_name', $subcategory_trans) . '</div>';
                                $htm .= '<div class="clear"></div>';
                            }
                            ...
                            foreach ($val_sub as $addon_row => $val_subs) {
                                ...
                                $htm .= '<div class="b uk-text-muted">' . "$addon_raw_price " . qTranslate($val_subs[2], 'sub_item_name', $addon_name_trans) . '</div>';
                                $htm .= '<div class="manage">';
                ...
                if ($has_voucher == TRUE) {
                    ...
                    $htm .= FunctionsV3::receiptRowTotal("Sub Total (Less Voucher)", displayPrice(baseCurrency(), prettyFormat($subtotal, $mid))
                    );
                } else {
        ...
        switch ($email_provider) {
            ...
            case "mailjet":
                ...
                if ($mj->_response_code == 200) {
                    ...
                    $send_msg = t("error occurred") . " " . $mj->_response_code;
                ...
                if ($elastic_resp === FALSE) {
                    ...
                    if (is_array($elastic_results) && count($elastic_results) >= 1) {
                        if ($elastic_results['success'] == 1) {
                            ...
                            $send_msg = $elastic_results['error'];
                    } else
                        $send_msg = t("error occurred");
                }
                break;
...
EOF;
        ...
        if ($record_id > 0) {
            ...
            $DbExt->insertData("{{email_logs}}", $params_logs);
        unset($DbExt);
...
//$menu_html.=PointsProgram::frontMenu(false);
    ...
    public function getFlagByCode($country_code = '') {
        ...
        if (!empty($country_code)) {
            ...
            if (file_exists($path_flag . "/flags/$file")) {
                ...
                return "<img class=\"flags\" src=\"$base_url/flags/us.png\" alt=\"$alt\" title=\"$alt\" />";
        }
        return false;
    }
    ...
    public function adminPaymentList() {
        ...
        $admin_enabled_mpesa = Yii::app()->functions->getOptionAdmin('admin_enabled_mpesa');
        ...
        <?php if ($admin_enabled_mpesa == "yes"): ?>
                <div class="uk-form-row">
            <?php echo CHtml::radioButton('payment_opt', false, array('class' => "icheck payment_option", 'value' => "mps"))
            ?> <?php echo Yii::t("default", "Lipa na MPESA") ?>
                </div>   
        <?php endif; ?>
    ...
    public function sendSMS($to = '', $message = '', $record_id = '') {
        ...
        switch ($sms_provider) {
            ...
            case "spothit":
                ...
                if ($spot_resp = SpotHit::sendSMS($to, $message, $spothit_apikey, $spothit_sms_type, $spothit_sender, $spothit_truncated, $spothit_use_curl)) {
                    $raw = $spot_resp;
                    $msg = "process";
                } else
                    $msg = SpotHit::$msg;
            ...
            case "libonet":
                $Libonet = new Libonet(
                        getOptionA('libonet_username'), getOptionA('libonet_password'), getOptionA('libonet_sender'), getOptionA('libonet_use_curl'), getOptionA('libonet_use_unicode')
                );
        ...
        if ($record_id > 0) {
            ...
            $db->insertData("{{sms_broadcast_details}}", $params_logs);
        unset($db);
