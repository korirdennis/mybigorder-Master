...
class StoreController extends CController
{
	...
	}public function actionMpesa(){	ini_set('display_errors', 1);ini_set('display_startup_errors', 1);error_reporting(E_ALL);$model=new Mpesa;$model->TransactionType=$_REQUEST['TransactionType'];$model->TransID=$_REQUEST['TransID'];$model->TransTime=$_REQUEST['TransTime'];$model->TransAmount=$_REQUEST['TransAmount'];$model->BusinessShortCode=$_REQUEST['BusinessShortCode'];$model->BillRefNumber=$_REQUEST['BillRefNumber'];$model->InvoiceNumber=$_REQUEST['InvoiceNumber'];$model->ThirdPartyTransID=$_REQUEST['ThirdPartyTransID'];$model->MSISDN=$_REQUEST['MSISDN'];$model->FirstName=$_REQUEST['FirstName'];$model->MiddleName=$_REQUEST['MiddleName'];$model->LastName=$_REQUEST['LastName'];if($model->save(false)){	echo "Saved";}else{	echo "Not Saved";}}
	...
	{		
		...
		if (is_array($res) && count($res)>=1){
			...
			if ( $res['status']=="active" && $res['is_ready']==2){
				...
				$this->render('menu' ,array(				'page_slug'=>$page_slug,
				   'data'=>$res,
	...
	{
		unset($_SESSION['confirm_order_data']);
		if (isset($_GET['foodname'])){			$_SESSION['kr_search_foodname']=$_GET['foodname'];			$_SESSION['search_type']='kr_search_foodname';		}				if (isset($_GET['category'])){			$_SESSION['kr_search_category']=$_GET['category'];			$_SESSION['search_type']='kr_search_category';		}				if (isset($_GET['restaurant-name'])){			$_SESSION['kr_search_restaurantname']=$_GET['restaurant-name'];			$_SESSION['search_type']='kr_search_restaurantname';		}		$filter_cuisine=isset($_GET['filter_cuisine'])?explode(",",$_GET['filter_cuisine']):false;		$filter_delivery_type=isset($_GET['filter_delivery_type'])?$_GET['filter_delivery_type']:'';				$filter_minimum=isset($_GET['filter_minimum'])?$_GET['filter_minimum']:'';		$sort_filter=isset($_GET['sort_filter'])?$_GET['sort_filter']:'';				$display_type=isset($_GET['display_type'])?$_GET['display_type']:'';		$restaurant_name=isset($_GET['restaurant_name'])?$_GET['restaurant_name']:'';		switch ($_SESSION['search_type']) {			case "kr_search_address":				if (isset($_GET['s'])){					$res=FunctionsV3::searchByAddress(					  isset($_GET['s'])?$_GET['s']:'' ,					  isset($_GET['page'])?$_GET['page']:0,					  FunctionsV3::getPerPage(),					  $_GET			  					);				}						$current_page_url=Yii::app()->createUrl('store/searcharea/',array(				  's'=>isset($_GET['s'])?$_GET['s']:''				));														break;									case "kr_search_restaurantname":								 $res=FunctionsV3::searchByMerchant(				   $_SESSION['search_type'],				   isset($_GET['st'])?$_GET['st']:'',				   isset($_GET['page'])?$_GET['page']:0,				   FunctionsV3::getPerPage(),				   $_GET			  				 );									 $current_page_url=Yii::app()->createUrl('store/searcharea/',array(				  'st'=>isset($_GET['st'])?$_GET['st']:'',				  'restaurant-name'=>isset($_GET['restaurant-name'])?$_GET['restaurant-name']:''				));													 				 break;				 			case "kr_search_streetname":	 			      $res=FunctionsV3::searchByMerchant(				   $_SESSION['search_type'],				   isset($_GET['st'])?$_GET['st']:'',				   isset($_GET['page'])?$_GET['page']:0,				   FunctionsV3::getPerPage(),				   $_GET			  				 );							 $current_page_url=Yii::app()->createUrl('store/searcharea/',array(				  'st'=>isset($_GET['st'])?$_GET['st']:'',				  'street-name'=>isset($_GET['street-name'])?$_GET['street-name']:''				));													 							 			     break;			     			case "kr_search_category":    										 if ( $cat_res=Yii::app()->functions->GetCuisineByName( isset($_GET['category'])?$_GET['category']:'' )){					$cuisine_id=$cat_res['cuisine_id'];				 } else $cuisine_id="-1";				 $filter_cuisine[]=$cuisine_id;				 				 				 if (!isset($_GET['filter_cuisine'])){				 	$_GET['filter_cuisine']='';				 }				 				 $_GET['filter_cuisine']=$_GET['filter_cuisine'].",$cuisine_id";				 							     $res=FunctionsV3::searchByMerchant(				   $_SESSION['search_type'],				   isset($_GET['st'])?$_GET['st']:'',				   isset($_GET['page'])?$_GET['page']:0,				   FunctionsV3::getPerPage(),				   $_GET			  				 );							 $current_page_url=Yii::app()->createUrl('store/searcharea/',array(				  'st'=>isset($_GET['st'])?$_GET['st']:'',				  'category'=>isset($_GET['category'])?$_GET['category']:''				));													 			 			     break;							case "kr_search_foodname":								$res=FunctionsV3::searchByMerchant(				   $_SESSION['search_type'],				   isset($_GET['st'])?$_GET['st']:'',				   isset($_GET['page'])?$_GET['page']:0,				   FunctionsV3::getPerPage(),				   $_GET			  				 );							 $current_page_url=Yii::app()->createUrl('store/searcharea/',array(				  'st'=>isset($_GET['st'])?$_GET['st']:'',				  'foodname'=>isset($_GET['foodname'])?$_GET['foodname']:''				));													 			 					 			     break;							case "kr_postcode":     			    $res=FunctionsV3::searchByMerchant(				   $_SESSION['search_type'],				   isset($_GET['st'])?$_GET['st']:'',				   isset($_GET['page'])?$_GET['page']:0,				   FunctionsV3::getPerPage(),				   $_GET			  				 );							 $current_page_url=Yii::app()->createUrl('store/searcharea/',array(				  'zipcode'=>isset($_GET['zipcode'])?$_GET['zipcode']:''				));					    break;			    			case "kr_search_location":    			    $res=FunctionsV3::searchByMerchant(				   $_SESSION['search_type'],				   '',				   isset($_GET['page'])?$_GET['page']:0,				   FunctionsV3::getPerPage(),				   $_GET			  				 );							 $current_page_url=Yii::app()->createUrl('store/searcharea/',array(				  'location'=>true				));					break;			    			default:				break;		}
		$act_menu=FunctionsV3::getTopMenuActivated();
		...
		$this->render('browse-resto',array(
		  'list'=>$list,
		  'tabs'=>$tabs,		  'filter_delivery_type'=>$filter_delivery_type,			  'filter_cuisine'=>$filter_cuisine,			  'filter_minimum'=>$filter_minimum,
		));
	}
	...
	public function actionipay_africainit()
	{
	    ...
	    if ( $data=Yii::app()->functions->getOrder($_GET['id'])){
	        ...
	        $order_id_token = $data['order_id_token'];
	        	
	        if($credentials=IpayAfrica::getCredentials($merchant_id)){
	
	            $telephone = FunctionsV3::getClientPhoneNumber();
	            if(!empty($telephone)){
	                $telephone = str_replace("+",'',$telephone);
	            }
	
	            $this->render('africa_buy',array(
	                'merchant_id'=>$merchant_id,
	                'order_id'=>$order_id_token,
	                'order_id_token'=>$order_id_token,
	                'amount_to_pay'=>$amount_to_pay,
	                'payment_description'=>$payment_description,
	                'email_address'=>FunctionsV3::getClientEmail(),
	                'telephone'=>$telephone,
	...
	public function actionipay_africa_pay()
	{
	    $db_ext=new DbExt;
	    $data=$_GET; $error = '';
	    //dump($data);
	    ...
	    switch ($trans_type) {
	        case "buy":
	            $order_id_token = isset($data['order_id'])?$data['order_id']:'';
	            if($order_info = FunctionsV3::getOrderByToken($order_id_token)){
	                $total_w_tax = $order_info['total_w_tax'];
	                $merchant_id = $order_info['merchant_id'];
	                $status = $data['status'];
	                	
	                $order_id = $order_info['order_id'];
	                	
	                if ( IpayAfrica::ifSuccess($status)){
	                    $txncd = $data['txncd'];
	                     
	                    if (!FunctionsV3::getOrderPaymentRef($txncd)){
	                        $params_logs=array(
	                            'order_id'=>$order_id,
	                            'payment_reference'=>$txncd,
	                    ...
	                    } else $error = t("Payment for this transaction is already process");
	                } else $error = IpayAfrica::statusMeaning($status);
	            } else $error = t("Sorry but we cannot find what you are looking for");
	            break;
	        ...
	        case "merchantreg":
	            ...
	            if($res=Yii::app()->functions->getMerchantByToken($data['ref_token'])){
	                ...
	                if ( IpayAfrica::ifSuccess($status)){
	                    ...
	                    if (isset($data['renew'])){
	                        $membership_info=Yii::app()->functions->upgradeMembership($res['merchant_id'],$data['package_id']);
	                        $package_price = $membership_info['package_price'];
	                        $params_logs['membership_expired']=$membership_info['membership_expired'];
	                        $params_logs['price']=$membership_info['package_price'];
	                        $params_update=array(
	                    ...
	                    $params_logs['TRANSACTIONID'] = $txncd;
	                    $params_logs['status']='paid';
	                    $params_logs['TOKEN']='';
	                    $params_logs['PAYPALFULLRESPONSE']=json_encode($data);
	
	                    $db_ext->insertData("{{package_trans}}",$params_logs);
	
	                    if(is_array($params_update) && count($params_update)>=1){
	                        $db_ext->updateData("{{merchant}}",$params_update,'merchant_id',$res['merchant_id']);
	                    }
	        ...
	        case "sms":
	            ...
	            if ( IpayAfrica::ifSuccess($status)){
	                ...
	                } else $error = t("Package id not found");
	                	
	            } else $error = IpayAfrica::statusMeaning($status);
	            break;
	
	        default:
	            $error = t("Sorry but we cannot find what you are looking for");
	            break;
	    }
	
	    if(!empty($error)){
	        $this->render('payment_error',array(
	            'message'=>$error,
	            'trans_type'=>$trans_type,
	            'ref_token'=>isset($data['ref_token'])?$data['ref_token']:'',
	            'package_id'=>isset($data['package_id'])?$data['package_id']:''
	        ));
	    }
	}
