...
{	
	...
	{		
		...
		switch ($search_mode) {
			...
			if ($this->data['address']=="null"){
				$this->data['address']="Parklands, Nairobi Kenya";
			}
		...
		if (isset($_GET['debug'])){
	 	...
	 	} else $this->msg=$this->t("No business found");
	...
	{	
		...
		if ( $data = AddonMobileApp::merchantInformation($this->data['merchant_id'])){				
			...
			$this->msg=$this->t("Successful");			
			$this->details=$data;			
		} else $this->msg=$this->t("business not found");
	...
	{		
		...
		'mri','hubtel');*/
		$mobile_payment=array('cod','paypal','pyr','pyp','atz','stp','rzr','obd','ocr','ipay_africa');
	...
	//		    'location_accuracy'=>!empty(getOptionA('mobileapp_location_accuracy'))?getOptionA('mobileapp_location_accuracy'):'false',
		        ...
		        'how_to_use_video'=>getOptionA('how_to_use_video')		    
		    ),
		     'translation'=>$lang
		    );
	...
	public function actioninitSearchCategory()
	{
		$DbExt=new DbExt; 
...
if (isset($this->data['cuisine_type'])){
			$and=" AND cuisine LIKE '%".$this->data['cuisine_type']."%'";
		}		
	...
	public function actiongetCategory()
	{
		...
		} else $this->msg = $this->t("This business has not published their menu yet.");
    ...
    public function actionipayInitiatorRequest()
    {
        /*GET CLIENT INFO*/
        ...
        $customer_email = $client['email_address'];
    
        if ( $order_info = FunctionsV3::getOrderByToken($order_id_token)){
            $order_id = $order_info['order_id'];
            $client_id = $order_info['client_id'];
            $merchant_id = $order_info['merchant_id'];
            $amount = normalPrettyPrice($order_info['total_w_tax']);
    
            if ($credentials = IpayAfrica::getCredentials($merchant_id)){
                //dump($credentials);
                ...
                $params = array(
                    ...
                    'tel'=>$contact_phone,
                    'eml'=>$customer_email,
                    'vid'=>$credentials['vendor_id'],
                    'curr'=>adminCurrencyCode(),
                    'cst'=>"0",
                );
                /*dump($params);
                 die();*/
    
                if ( $resp=IpayAfrica::initiatorRequest($params,$hashkey)){
                    	
                    $business_code = array(); $instructions=array();
                    foreach ($resp['payment_channels'] as $val) {
                        $business_code[$val['name']]=$val['paybill'];
                        ...
                        switch ($val['name']) {
                            case "MPESA":
                                $html.="<ul>";
                                $html.="<li>Click on the <b>PAY NOW</b> button to generate a payment request on your phone</li>";
                                $html.="<li>Enter your <b>MPESA PIN</b> on your phone to complete the payment</li>";
                                $html.="<li>You will get your receipt from MPESA and an SMS confirmation from iPay</li>";
                                $html.="<li> Didn't get the prompt on your phone? Kindly dial <b>*234*1*6#</b> to force SIM update. For SIM cards <b>more than 2 years old</b> a SIM swap may be necessary</li>";
                                $html.="</ul>";
                                break;
    
                            case "AIRTEL":
                                $html.="<ul class=\"list-dim-text\">";
                                $html.="<li>Go to the Airtel Money menu on your phone</li>";
                                $html.="<li>Select To Make Payments</li>";
                                $html.="<li>Select Pay Bill Option</li>";
                                $html.="<li>Select Others</li>";
                                $html.="<li>Enter the Business name <b>".$val['paybill']."</b></li>";
                                $html.="<li>Enter the <b>EXACT</b> amount (".FunctionsV3::prettyPrice($resp['amount']).") </li>";
                                $html.="<li>Enter your PIN</li>";
                                $html.="<li>Enter <b>".$resp['account']."</b> as the Reference and then send the money</li>";
                                $html.="<li>You will receive a transaction confirmation SMS from Airtel Money</li>";
                                $html.="<li>Then click on the Confirm Payment Done button below</li>";
                                $html.="</ul>";
                                break;
    
                            case "EQUITEL":
                                $html.="<ul class=\"list-dim-text\">";
                                $html.="<li>Dial *126# and select Lipa na Bonga Points.</li>";
                                $html.="<li>Select Pay Bill</li>";
                                $html.="<li>Enter ".$val['paybill']." as the Paybill</li>";
                                $html.="<li>Enter Account Number (".$resp['account'].")</li>";
                                $html.="<li>Enter the EXACT Amount ".FunctionsV3::prettyPrice($resp['amount'])."</li>";
                                $html.="<li>Please confirm payment of ".FunctionsV3::prettyPrice($resp['amount'])." worth to iPay Ltd</li>";
                                $html.="<li>Enter your Bonga Points PIN</li>";
                                $html.="<li>You will receive a transaction confirmation SMS</li>";
                                $html.="<li>Then click on the Confirm Payment Done button below</li>";
                                $html.="</ul>";
                                break;
                    ...
                    $this->details = array(
                        ...
                        'amount'=>$resp['amount'],
                        'hash'=>$resp['hash'],
                        'payment_reference'=>$resp['account'],
                        'business_code'=>$business_code,
                        'instructions'=>$instructions
                    );
                } else $this->msg = AddonMobileApp::t(IpayAfrica::$message);
    
            } else $this->msg = AddonMobileApp::t("Credentials is not yet");
    
        } else $this->msg = AddonMobileApp::t("Order information not found");
        $this->output();
    }
    
    public function actionipaympesaSendCode()
    {
        $ipay_phone_number = isset($this->data['ipay_phone_number'])?trim($this->data['ipay_phone_number']):'';
        if(!empty($ipay_phone_number)){
    
            $ipay_phone_number = str_replace("+","",$ipay_phone_number);
            ...
            $order_id_token = isset($this->data['ipay_oid'])?$this->data['ipay_oid']:'';
            ...
            if ( $order_info = FunctionsV3::getOrderByToken($order_id_token)){
                ...
                if ($credentials = IpayAfrica::getCredentials($merchant_id)){
                    $vid = $credentials['vendor_id'];
                    $sid = $this->data['ipay_sid'];
                    $datastring = $ipay_phone_number.$vid.$sid;
                    $generated_hash = hash_hmac('sha256',$datastring , $credentials['hashkey']);
                    $params = array(
                        'phone'=>$ipay_phone_number,
                        'sid'=>$this->data['ipay_sid'],
                        'vid'=>$credentials['vendor_id'],
                        'hash'=>$generated_hash
                    );
                    ...
                    if ($resp = IpayAfrica::call(IpayAfrica::$api_stk,$content)){
                        if ($resp['status']==1){
                            $this->code = 1;
                            $this->msg = $resp['text'];
                        } else if ($resp['status']==400) {
                            $this->msg = IpayAfrica::parseError( $resp['error'] );
                        } else $this->msg = "invalid response from api";
                    } else $this->msg = IpayAfrica::$message;
                } else $this->msg = AddonMobileApp::t("Credentials is not yet");
                 
            } else $this->msg = AddonMobileApp::t("Order information not found");
    
        } else $this->msg = AddonMobileApp::t("mobile number is required");
        $this->output();
    }
    
    public function actionipayMobileMoney()
    {
        ...
        $Validator=new Validator;
        $req=array(
            'ipay_sid'=>$this->t("sid is required"),
            'ipay_hash'=>$this->t("hash is required"),
            'ipay_oid'=>$this->t("order id is required"),
        );
        ...
        if ($Validator->validate()){
            ...
            $order_id_token = isset($this->data['ipay_oid'])?trim($this->data['ipay_oid']):'';
            ...
            if ( $order_info = FunctionsV3::getOrderByToken($order_id_token)){
                ...
                if ($credentials = IpayAfrica::getCredentials($merchant_id)){
                    $hashkey=$credentials['hashkey'];
                    $datastring = $this->data['ipay_sid'].$credentials['vendor_id'];
                    ...
                    $content = http_build_query($params);
                    if ($resp = IpayAfrica::call(IpayAfrica::$api_mobilemoney,$content)){
    
                        //$resp['status'] ='aei7p7yrx4ae34'; /*TEST SUCCESS*/
    
                        if ($resp['status']=="400"){
                            $this->msg = IpayAfrica::parseError($resp['error']);
                            	
                        } else if ($resp['status']=="0") {
                            $this->msg = $this->t("failed").". ".$resp['text'];
                             
                        } else {
                            if ( IpayAfrica::ifSuccess($resp['status'])){
                                ...
                                $this->details=array(
                                    'next_step'=>"receipt",
                                    'amount_to_pay'=>$this->data['ipay_amount']
                                );
                                	
                                $db_ext=new DbExt;
                                $params_logs=array(
                                    'order_id'=>$order_id,
                                    'payment_type'=>"ipay_africa",
                                    'raw_response'=>json_encode($resp),
                                    'date_created'=>AddonMobileApp::dateNow(),
                                    'ip_address'=>$_SERVER['REMOTE_ADDR'],
                                    'payment_reference'=>$resp['qwh']
                                );
                            ...
                            } else $this->msg = IpayAfrica::statusMeaning($resp['status']);
                        }
                    } else $this->msg = IpayAfrica::$message;
                } else $this->msg = $this->t("Credentials is not yet");
    
            } else $this->msg = $this->t("Order information not found");
             
        } else $this->msg=AddonMobileApp::parseValidatorError($Validator->getError());
        $this->output();
    }
    
    public function actionipayTransactCC()
    {
        //$this->data['cc_number']='44';
        //dump($this->data);
        ...
        if(!is_numeric($this->data['cc_number'])){
            $Validator->msg[]=$this->t("Invalid card number");
        }
    
        $len =strlen($this->data['cc_number']);
        if($len<=15){
            $Validator->msg[]=$this->t("Invalid card number");
        }
            ...
            if ( $order_info = FunctionsV3::getOrderByToken($order_id_token)){
                ...
                if ($credentials = IpayAfrica::getCredentials($merchant_id)){
                    ...
                    $params = array(
                        ...
                        'curr'=>adminCurrencyCode(),
                        'cvv'=>$this->data['cvv'],
                        'cardno'=>$this->data['cc_number'],
                        'month'=>$this->data['expiration_month'],
                        'year'=>substr($this->data['expiration_yr'],-2,2),
                        'sid'=>$this->data['ipay_sid'],
                        'fname'=>$this->data['first_name'],
                        'lname'=>$this->data['last_name'],
                    );
                     
                    $datastring = $this->data['ipay_sid'].$params['vid'].$params['cardno'].$params['cvv'].$params['month'].$params['year'].$params['fname'].$params['lname'];
                    $generated_hash = hash_hmac('sha256',$datastring , $hashkey);
                    ...
                    if ($resp = IpayAfrica::call(IpayAfrica::$api_cc,$content)){
                        if ($resp['status']=="400"){
                            $this->msg = IpayAfrica::parseError($resp['error']);
                        } else if ($resp['status']=="0") {
                            $this->msg = $this->t("failed").". ".$resp['text'];
                        } else {
                            //if($resp['status']=="bdi6p2yy76etrs" || $resp['status']=="aei7p7yrx4ae34" ){
                            if($resp['status']=="aei7p7yrx4ae34" ){
                    ...
                    } else $this->msg = $this->t(IpayAfrica::$message);
